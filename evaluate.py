...
def plot_cause_cm(y_true, y_pred, title):
    # For multi-label, we show a matrix where [i, j] is count of times 
    # Label i was true and Label j was predicted. Diagonals are Correct hits.
    ...
    # Normalize by True count for each Lag
...
def run_evaluation():
    ...
    evaluate_set(DataLoader(dev_ds, batch_size=32), load_model_for_eval(device), device)
