...
def run_test_inference():
    ...
    test_ds = MECPEDataset(os.path.join(CONFIG['base_path'], 'test_sent_emo.csv'),
                           os.path.join(CONFIG['base_path'], 'test.json'), 
    ...
    plot_cm(all_labels_e, all_preds_e, "Emotion CM (Test)", emotion_names, cmap='Blues')
    
    print("\nCAUSAL SPAN EXTRACTION (Test Set)")
    print("="*50)
    if len(all_labels_c) > 0:
        ...
        plot_cm(all_labels_c, all_preds_c, "Cause CM (Test)", names, cmap='Greens')
        
        # Calculate and print combined F1, as it was in the original code
        emo_f1 = f1_score(all_labels_e, all_preds_e, average='macro')
        cause_f1 = f1_score(all_labels_c, all_preds_c, average='macro')
        print(f"\nCombined F1: {(emo_f1+cause_f1)/2:.4f}")
    else:
